# -*- coding: utf-8 -*-
"""
Automation script to render and SCHEDULE remaining 9 Reels (07 to 15)
across Thursday 3 Sep, Friday 4 Sep, and Saturday 5 Sep 2026.
"""
import os
import subprocess
...
# Remaining 9 Reels to schedule (Reel 07 to 15)
SCHEDULE_CONFIG = [
    {
...
#Reels #MASTERม1 #คณิตศาสตร์ #สูตรลัด #สอบเข้าม1 #ONET #สสวท #Gifted""",
        # Thursday 3 Sep 20:30 Thai (13:30 UTC)
        "sched_dt": datetime.datetime(2026, 9, 3, 13, 30, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "พฤหัสบดี 3 ก.ย. 20:30 น."
    },
    {
...
#Reels #MASTERม1 #วิทยาศาสตร์ #ความร้อน #สอบเข้าม1 #ONET #Edutainment""",
        # Friday 4 Sep 07:30 Thai (00:30 UTC)
        "sched_dt": datetime.datetime(2026, 9, 4, 0, 30, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "ศุกร์ 4 ก.ย. 07:30 น."
    },
    {
...
#Reels #MASTERม1 #ฟิสิกส์ #วิทยาศาสตร์ #สมดุลความร้อน #Gifted #สสวท""",
        # Friday 4 Sep 12:15 Thai (05:15 UTC)
        "sched_dt": datetime.datetime(2026, 9, 4, 5, 15, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "ศุกร์ 4 ก.ย. 12:15 น."
    },
    {
...
#Reels #MASTERม1 #ภาษาอังกฤษ #IfClause #Grammar #สอบเข้าม1 #ONET""",
        # Friday 4 Sep 17:45 Thai (10:45 UTC)
        "sched_dt": datetime.datetime(2026, 9, 4, 10, 45, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "ศุกร์ 4 ก.ย. 17:45 น."
    },
    {
...
#Reels #MASTERม1 #ภาษาอังกฤษ #IfClause #จุดลวงสทศ #สอบเข้าม1 #ONET""",
        # Friday 4 Sep 20:30 Thai (13:30 UTC)
        "sched_dt": datetime.datetime(2026, 9, 4, 13, 30, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "ศุกร์ 4 ก.ย. 20:30 น."
    },
    {
...
#Reels #MASTERม1 #ภาษาไทย #คำสมาส #สอบเข้าม1 #ONET #เตรียมสอบม1""",
        # Saturday 5 Sep 09:00 Thai (02:00 UTC)
        "sched_dt": datetime.datetime(2026, 9, 5, 2, 0, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "เสาร์ 5 ก.ย. 09:00 น."
    },
    {
...
#Reels #MASTERม1 #ภาษาไทย #คำสนธิ #สอบเข้าม1 #ONET #เตรียมสอบม1""",
        # Saturday 5 Sep 12:00 Thai (05:00 UTC)
        "sched_dt": datetime.datetime(2026, 9, 5, 5, 0, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "เสาร์ 5 ก.ย. 12:00 น."
    },
    {
...
#Reels #MASTERม1 #คณิตศาสตร์ #พีทาโกรัส #สอบเข้าม1 #ONET #เตรียมสอบม1""",
        # Saturday 5 Sep 15:30 Thai (08:30 UTC)
        "sched_dt": datetime.datetime(2026, 9, 5, 8, 30, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "เสาร์ 5 ก.ย. 15:30 น."
    },
    {
...
#Reels #MASTERม1 #คณิตศาสตร์ #พีทาโกรัส #สูตรลัด3วิ #สอบเข้าม1 #Gifted""",
        # Saturday 5 Sep 19:00 Thai (12:00 UTC)
        "sched_dt": datetime.datetime(2026, 9, 5, 12, 0, 0, tzinfo=datetime.timezone.utc),
        "sched_th": "เสาร์ 5 ก.ย. 19:00 น."
    }
]
...
def render_short(cfg):
    ...
    # If already rendered and valid, return existing
    if os.path.exists(out_file) and os.path.getsize(out_file) > 1000000:
        print(f"⏩ [Cache] Found pre-rendered file: {out_file} ({os.path.getsize(out_file):,} bytes)", flush=True)
        return out_file
...
def schedule_reel(file_path, title, caption, sched_dt, sched_th):
    file_size = os.path.getsize(file_path)
    sched_timestamp = int(sched_dt.timestamp())
    print(f"\n🚀 [Phase 1/3] Initializing Scheduled Reel ({sched_th})...", flush=True)
    ...
    # Phase 3: Schedule
    print(f"⏰ [Phase 3/3] Setting Schedule for: {sched_th} (Timestamp: {sched_timestamp})...", flush=True)
    ...
    post_id = res_finish.get('post_id', video_id)
    print(f"🎉 Reel successfully SCHEDULED for {sched_th}! Post ID: {post_id}", flush=True)
    return post_id

print(f"🔥 Starting Render & Scheduling Pipeline for Remaining 9 Reels...", flush=True)
results = []

for i, cfg in enumerate(SCHEDULE_CONFIG, start=7):
    print(f"\n=======================================================", flush=True)
    print(f"[{i}/15] Processing Reel #{i}: {cfg['title']} ➔ {cfg['sched_th']}", flush=True)
    print(f"=======================================================", flush=True)
    out_mp4 = render_short(cfg)
    if out_mp4 and os.path.exists(out_mp4):
        try:
            pid = schedule_reel(out_mp4, cfg['title'], cfg['caption'], cfg['sched_dt'], cfg['sched_th'])
            results.append((cfg['id'], cfg['sched_th'], pid))
            time.sleep(2)
        except Exception as e:
            print(f"❌ Error scheduling {cfg['id']}: {e}", flush=True)
    else:
        print(f"⚠️ Skipping scheduling for {cfg['id']} due to render error.", flush=True)

print(f"\n🏆 All Remaining Reels Scheduled Successfully!", flush=True)
print(f"Total Scheduled in this batch: {len(results)}")
for cid, th_time, pid in results:
    print(f"  • {cid} | เวลา: {th_time} | ID: {pid}", flush=True)
